...
genetic\footnote{\url{publications.nigms.nih.gov/structlife/images/ch1_str12_big.jpg}} regulatory networks, the design of robust and scalable communication
...
\begin{description}
  \item[Average Degree] See section \ref{sec:degreenode}. Implementation is
    trivial.
  \item[Average Distance] See section \ref{sec:distance}. Implemented using a
    breadth first search for the shortest path from each node (which is a
    special case of Dijkstra's algorithm).
  \item[Clustering Average] Implemented by calculating the clustering
    coefficient (see section \ref{sec:clusteringcoefficient}) for every node and
    then calculating the average.
  \item[Scale Free Exponent] Implemented calculating the degree distribution of
    the graph (see section \ref{sec:scale-free}), and then getting the gradient
    of its plot.
  \item[Clustering Coefficient] See section
    \ref{sec:clusteringcoefficient}. This statistic is shown for a single node
    when hovering over it. Implemented getting the intersection of the node's
    neighbours and its neighbours' neighbours; and then multiplying the size of
    this set by $\frac{2}{k(k-1)}$, where $k$ is the degree of the node.
\end{description}
...
=======
    \REQUIRE An array of all nodes exluding $n_i$ called Nodes
>>>>>>> c8d564b195e2168f038e05ada3edb29a43ab5025
    \WHILE{Edge count $>$ 0}
      \STATE r $\gets$ random real in $[0, 1]$
...
\begin{algorithmic}
\REQUIRE An array of nodes of set U called uVector
\REQUIRE An array of nodes of set V called vVector
\REQUIRE A degree distribution for all nodes

\STATE Populate uVector and vVector
\FORALL{Elements of uVector}
  \STATE Connect to random members of vVector. The degrees of the connections are governed
              by the degree distribution.
\ENDFOR
\STATE Connect all disconnected members of uVector to a random member in vVector
\FORALL{Elements of vVector $V_i$}
  \STATE Connect all members of uVector connected to $V_i$
\ENDFOR
\STATE Remove all edges associated with vVector.
\STATE Remove all vVector members.
\end{algorithmic}
...
kind of problems we encountered are:
\begin{itemize}
...
Hence, each member of the team was constantly challenged
individually. We also organised short technical talks to
introduce concepts that may be unfamiliar to some and which they may
find useful; we have done such talks on \Git and branching hygiene
as well as \buzz{C++} and \Qt.


\subsection{Planning (or lack thereof)}
...
development. Since it is built wholly on open-source libraries, there
...
very proud of the result and we plan to release it to the wider public in the
near future.
...
code base. We went even further than that and proposed features of our own which
were all implemented. For this reason we are satisfied not only with the end
result, but also with how smooth the development process was, and we think our
client is satisfied as well.
...
\begin{center}
  \noindent\begin{minipage}{.45\textwidth}
    ...
    \captionof{figure}{Barab\'{a}si-Albert}
  \end{minipage}
  \begin{minipage}{.45\textwidth}
    \centering
    \includegraphics[width=5cm]{bipartite.png}
    \captionof{figure}{Bipartite}
  \end{minipage}
  \medskip
  ...
  \noindent\begin{minipage}{.45\textwidth}
    ...
    \captionof{figure}{Erd\H{o}s-R\'{e}nyi}
  \end{minipage}
  \begin{minipage}{.45\textwidth}
    \centering
    \includegraphics[width=5cm]{preferential.png}
    \captionof{figure}{Preferential attachment with clustering}
  \end{minipage}
  \medskip

  \noindent\begin{minipage}{.45\textwidth}
    \centering
    \includegraphics[width=5cm]{watts.png}
    \captionof{figure}{Watts-Strogatz}
  \end{minipage}
  \begin{minipage}{.45\textwidth}
    \centering
    \includegraphics[width=5cm]{twitter.png}
