...
World Networks and hence can be used as an introduction to them. By
...
the properties of Small World Networks. The network generation
algorithms in Visigoth form a smooth progression in the number of
...
analysis. As such it has only become feasible to study real networks
with their large underlying data sets within the last decade.
\cite{oconn11}
...
In order to easen further analysis of real networks, the aim is to
simulate them using procedurally generated networks. However, this
requires the generation algorithms used to output networks which
are true to the characteristic structural properties of \emph{Small
World Networks}.
...
``small world phenomenon'' \cite{swphen} of strangers
being linked by a mutual acquaintance.

Social networks, neural networks, the World Wide Web, electrical
power grids and protein structures are all examples for complex networks
the precise dynamic behaviour of which currently escapes our understanding.
...
A \emph{network} is a graph, which in turn is an ordered pair $G(N,E)$
comprising of a set of \emph{nodes} $N$ and a set of \emph{edges}, $E$.
An \emph{edge} is a 2-element subset of $N$ representing a
\emph{connection} between a pair of \emph{nodes}.

The \emph{degree} $K_i$ of a node $i$ is the number of nodes that it is
connected to. The average degree $k$ of a network is $k =\frac{2 \cdot m}{|N|}$
where $m$ is the number of edges in the network and $|N|$ is the total
number of nodes.
...
A group of people can be represented by a \emph{social network}, a graph
in which individuals are the nodes and an edge betwen them represents
an acquaintance.

In such a graph, a sequence of edges from one individual to another is
called a path, and the length of the shortest possible path is of
special interest as it tends to be surprisingly low in natural networks.

Frigyes Karinthy first introduced the idea of ``six degrees of separation''
in his 1929 short story ``Chains''.

The small world experiment, performed in the 1960s by Stanley Milgram
and other researchers,
formalised the aforementioned concept. In this experiment, letters were
...
between almost any two nodes within them. This follows from the
defining property of a high clustering coefficient.

Secondly, most pairs of nodes will be connected by at least one
short path. This follows from the defining property that the
mean-shortest path length has to be small.
...
connections mediating the short path lengths between other
edges. By analogy, the small-world network of airline flights
has a small mean-path length (i.e. between any two cities you
are likely to have to take three or fewer flights) because many
...
grids, metabolite processing networks, networks of brain
...
\emph{transcriptional networks}, in which the nodes are genes,
and they are linked if one gene has an up or down-regulatory
genetic influence on the other, have small world network properties.
...
repertoire. The simplicity of this algorithm is attractive, however
...
\emph{short average path length} and \emph{high clustering}.
...
networks, as described in section \ref{sec:scale-free}. This is achieved
using preferential attachment, meaning that nodes are added to the graph
one by one and selected for attachment with a probability proportional
to their degree in the network. This model demonstrates
...
Such networks are present in both natural and human made systems;
cf. the networks in the quoted works.

A network is deemed scale-free if the network exhibits a power
...
are also \emph{scale invariant} meaning that their size has no effect on
their attributes (i.e. degree distribution).
...
The average path length of Barabasi-Albert networks is comparable
to the average path length of rd\H{o}s-R\'{e}nyi networks of equal
size and average degree.

The clustering coefficient of Barabasi-Albert networks is greater
than the clustering coefficient of rd\H{o}s-R\'{e}nyi networks of
equal size and average degree \cite{oconn11}, but relatively low
...
%FIXME: please fix the doc layout
...
\begin{enumerate}
  ...
  \begin{algorithmic}
    ...
    \FORALL{edges $E_i$ in $S$}
      \FORALL{edges $E_j$ in $S$}
        \IF{$E_i$ and $E_j$ share the same node in $U$}
...
The Barabasi-Albert model has been used frequently to model real
...
In the original Barabasi-Albert model, a new vertex is added to
